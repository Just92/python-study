...
if __name__=="__main__":
	...
	pfile.alter_ini("ck_uac", "release", release_value,file=ini_file)
	pfile.alter_ini("ck_uac", "build", build_value,file=ini_file)
	#设置常量参数
	cf = ConfigParser.ConfigParser()
	cf.read(ini_file)
	release = unicode(cf.get("ck_uac", "release"))
	build = unicode(cf.get("ck_uac", "build"))
	vss_prj = unicode(cf.get("ck_uac", "project"))
	vss_ldir = unicode(cf.get("ck_uac", "localdir"))
	ck_file = unicode(cf.get("ck_uac", "file"))
	fname = unicode(cf.get("ck_uac", "fpath"))
	sname = unicode(cf.get("ck_uac", "sname"))
	root_path = unicode(cf.get("ck_uac", "rootpath"))
